  ...
  function initHeroSlider(container = "[data-hero-slider]") {
    ...
    if (videoUrl) {
      ...
      if (vimeoId) {
        ...
        const cmsThumb = document.querySelector("img[data-hero-video-thumb]");
        if (cmsThumb?.src) {
          appendThumb(cmsThumb.src);
        } else {
          fetch(`https://vimeo.com/api/oembed.json?url=${encodeURIComponent(`https://vimeo.com/${vimeoId}`)}`).then((r) => r.json()).then((data) => {
