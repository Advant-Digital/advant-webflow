...
(() => {
  ...
  function getVimeoUrl() {
    var oEmbedParameters3 = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {};
    var id = oEmbedParameters3.id;
    var url = oEmbedParameters3.url;
    var idOrUrl = id || url;
    if (!idOrUrl) {
  ...
  function initializeScreenfull() {
    ...
    var screenfull3 = {
      request: function request(element) {
        return new Promise(function(resolve, reject) {
          var onFullScreenEntered = function onFullScreenEntered2() {
            screenfull3.off("fullscreenchange", onFullScreenEntered2);
            resolve();
          };
          screenfull3.on("fullscreenchange", onFullScreenEntered);
      ...
      exit: function exit() {
        return new Promise(function(resolve, reject) {
          if (!screenfull3.isFullscreen) {
            resolve();
            return;
          }
          var onFullScreenExit = function onFullScreenExit2() {
            screenfull3.off("fullscreenchange", onFullScreenExit2);
            resolve();
          };
          screenfull3.on("fullscreenchange", onFullScreenExit);
  ...
  var isServerRuntime = isNode2 || isBun || isDeno || isCloudflareWorker;
  function getMethodName2(prop, type) {
    if (prop.indexOf(type.toLowerCase()) === 0) {
      return prop;
    }
  ...
  function isVimeoUrl2(url) {
    return /^(https?:)?\/\/((((player|www)\.)?vimeo\.com)|((player\.)?[a-zA-Z0-9-]+\.(videoji\.(hk|cn)|vimeo\.work)))(?=$|\/)/.test(url);
  }
  function isVimeoEmbed2(url) {
  ...
  function getVimeoUrl2() {
    let oEmbedParameters3 = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {};
    const id = oEmbedParameters3.id;
    const url = oEmbedParameters3.url;
    const idOrUrl = id || url;
    if (!idOrUrl) {
      throw new Error("An id or url must be passed, either in an options object or as a data-vimeo-id or data-vimeo-url attribute.");
    }
    if (isInteger2(idOrUrl)) {
      return `https://vimeo.com/${idOrUrl}`;
    }
    if (isVimeoUrl2(idOrUrl)) {
      return idOrUrl.replace("http:", "https:");
    }
    if (id) {
  ...
  var arrayIndexOfSupport2 = typeof Array.prototype.indexOf !== "undefined";
  var postMessageSupport2 = typeof window !== "undefined" && typeof window.postMessage !== "undefined";
  if (!isServerRuntime && (!arrayIndexOfSupport2 || !postMessageSupport2)) {
    throw new Error("Sorry, the Vimeo Player API is not available in this browser.");
  }
  ...
  })(typeof globalThis !== "undefined" ? globalThis : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : typeof commonjsGlobal2 !== "undefined" ? commonjsGlobal2 : commonjsGlobal2);
  var npo_src2 = createCommonjsModule2(function(module) {
    (function UMD(name, context, definition) {
      context[name] = context[name] || definition();
      if (module.exports) {
        module.exports = context[name];
      }
    })("Promise", typeof commonjsGlobal2 != "undefined" ? commonjsGlobal2 : commonjsGlobal2, function DEF() {
      var builtInProp, cycle, scheduling_queue, ToString = Object.prototype.toString, timer = typeof setImmediate != "undefined" ? function timer2(fn) {
        return setImmediate(fn);
      } : setTimeout;
      try {
  ...
  var callbackMap2 = /* @__PURE__ */ new WeakMap();
  function storeCallback2(player, name, callback) {
    const playerCallbacks = callbackMap2.get(player.element) || {};
    if (!(name in playerCallbacks)) {
      playerCallbacks[name] = [];
    }
    playerCallbacks[name].push(callback);
    callbackMap2.set(player.element, playerCallbacks);
  }
  function getCallbacks2(player, name) {
    const playerCallbacks = callbackMap2.get(player.element) || {};
    return playerCallbacks[name] || [];
  }
  function removeCallback2(player, name, callback) {
    const playerCallbacks = callbackMap2.get(player.element) || {};
    if (!playerCallbacks[name]) {
      return true;
    }
  ...
  function shiftCallbacks2(player, name) {
    const playerCallbacks = getCallbacks2(player, name);
    if (playerCallbacks.length < 1) {
      return false;
    }
    const callback = playerCallbacks.shift();
    removeCallback2(player, name, callback);
    return callback;
  }
  function swapCallbacks2(oldElement, newElement) {
    const playerCallbacks = callbackMap2.get(oldElement);
    callbackMap2.set(newElement, playerCallbacks);
    callbackMap2.delete(oldElement);
  }
  function parseMessageData2(data) {
    if (typeof data === "string") {
      try {
        data = JSON.parse(data);
      } catch (error2) {
  ...
  function postMessage2(player, method, params) {
    if (!player.element.contentWindow || !player.element.contentWindow.postMessage) {
      return;
    }
    let message = {
  ...
  function processData2(player, data) {
    data = parseMessageData2(data);
    let callbacks = [];
    let param;
    if (data.event) {
      if (data.event === "error") {
        const promises = getCallbacks2(player, data.data.method);
        promises.forEach((promise) => {
          const error2 = new Error(data.data.message);
          error2.name = data.data.name;
          promise.reject(error2);
          removeCallback2(player, data.data.method, promise);
        });
      }
      callbacks = getCallbacks2(player, `event:${data.event}`);
      param = data.data;
    } else if (data.method) {
      const callback = shiftCallbacks2(player, data.method);
      if (callback) {
        callbacks.push(callback);
        param = data.value;
      }
  ...
  var oEmbedParameters2 = ["airplay", "audio_tracks", "audiotrack", "autopause", "autoplay", "background", "byline", "cc", "chapter_id", "chapters", "chromecast", "color", "colors", "controls", "disable_context_menu", "dnt", "end_time", "fullscreen", "height", "id", "initial_quality", "interactive_params", "keyboard", "loop", "maxheight", "max_quality", "maxwidth", "min_quality", "muted", "play_button_position", "playsinline", "portrait", "preload", "progress_bar", "quality", "quality_selector", "responsive", "skipping_forward", "speed", "start_time", "texttrack", "thumbnail_id", "title", "transcript", "transparent", "unmute_button", "url", "vimeo_logo", "volume", "watch_full_video", "width"];
  function getOEmbedParameters2(element) {
    let defaults = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
    return oEmbedParameters2.reduce((params, param) => {
  ...
  function createEmbed2(_ref, element) {
    let {
      html
    } = _ref;
    if (!element) {
  ...
  function initializeEmbeds2() {
    ...
    const handleError = (error2) => {
      if ("console" in window && console.error) {
        console.error(`There was an error creating an embed: ${error2}`);
      }
    };
    elements.forEach((element) => {
      try {
        ...
        const params = getOEmbedParameters2(element);
        const url = getVimeoUrl2(params);
        getOEmbedData2(url, params, element).then((data) => {
          return createEmbed2(data, element);
        }).catch(handleError);
      } catch (error2) {
        handleError(error2);
      }
  ...
  function initAppendVideoMetadata2() {
    let parent = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : document;
    if (window.VimeoSeoMetadataAppended) {
      return;
    }
    ...
    const onMessage = (event) => {
      ...
      const data = parseMessageData2(event.data);
      if (!data || data.event !== "ready") {
        return;
      }
      const senderIFrame = event.source ? findIframeBySourceWindow(event.source, parent) : null;
      if (senderIFrame && isVimeoEmbed2(senderIFrame.src)) {
        const player = new Player2(senderIFrame);
        player.callMethod("appendVideoMetadata", window.location.href);
      }
    };
    window.addEventListener("message", onMessage);
  ...
  function checkUrlTimeParam() {
    ...
    const handleError = (error2) => {
      if ("console" in window && console.error) {
        console.error(`There was an error getting video Id: ${error2}`);
      }
    };
    const onMessage = (event) => {
      if (!isVimeoUrl2(event.origin)) {
      ...
      if (senderIFrame && isVimeoEmbed2(senderIFrame.src)) {
        ...
        player.getVideoId().then((videoId) => {
          const matches3 = new RegExp(`[?&]vimeo_t_${videoId}=([^&#]*)`).exec(window.location.href);
          if (matches3 && matches3[1]) {
            const sec = decodeURI(matches3[1]);
  ...
  function initializeScreenfull2() {
    const fn = function() {
      let val;
      const fnMap = [
        ["requestFullscreen", "exitFullscreen", "fullscreenElement", "fullscreenEnabled", "fullscreenchange", "fullscreenerror"],
      ...
      request(element) {
        return new Promise((resolve, reject) => {
          const onFullScreenEntered = function() {
            screenfull3.off("fullscreenchange", onFullScreenEntered);
            resolve();
          };
          screenfull3.on("fullscreenchange", onFullScreenEntered);
          element = element || document.documentElement;
      ...
      exit() {
        return new Promise((resolve, reject) => {
          ...
          const onFullScreenExit = function() {
            screenfull3.off("fullscreenchange", onFullScreenExit);
            resolve();
          };
          screenfull3.on("fullscreenchange", onFullScreenExit);
    ...
    maintainPlaybackPosition(timingObject, player, options) {
      ...
      const check = async () => {
        ...
        if (diffAbs > maxAllowedDrift) {
          ...
          const min3 = diffAbs / maxTimeToCatchUp;
          const max3 = maxRateAdjustment;
          const adjustment = min3 < max3 ? (max3 - min3) / 2 : max3;
  ...
  var playerMap2 = /* @__PURE__ */ new WeakMap();
  var readyMap2 = /* @__PURE__ */ new WeakMap();
  var screenfull2 = {};
  var Player2 = class {
    /**
     * Create a Player.
    ...
    constructor(element) {
      ...
      if (element.nodeName === "IFRAME" && !isVimeoUrl2(element.getAttribute("src") || "")) {
        throw new Error("The player element passed isn\u2019t a Vimeo embed.");
      }
      if (playerMap2.has(element)) {
        return playerMap2.get(element);
      }
      this._window = element.ownerDocument.defaultView;
      this.element = element;
      this.origin = "*";
      const readyPromise = new npo_src2((resolve, reject) => {
        this._onMessage = (event) => {
          if (!isVimeoUrl2(event.origin) || this.element.contentWindow !== event.source) {
            return;
          }
          if (this.origin === "*") {
          ...
          processData2(this, data);
        };
        this._window.addEventListener("message", this._onMessage);
        if (this.element.nodeName !== "IFRAME") {
          const params = getOEmbedParameters2(element, options);
          const url = getVimeoUrl2(params);
          getOEmbedData2(url, params, element).then((data) => {
            const iframe = createEmbed2(data, element);
            this.element = iframe;
            this._originalElement = element;
            swapCallbacks2(element, iframe);
            playerMap2.set(this.element, this);
            return data;
          }).catch(reject);
        }
      });
      readyMap2.set(this, readyPromise);
      playerMap2.set(this.element, this);
      if (this.element.nodeName === "IFRAME") {
        postMessage2(this, "ping");
      }
      if (screenfull2.isEnabled) {
        const exitFullscreen = () => screenfull2.exit();
        this.fullscreenchangeHandler = () => {
          if (screenfull2.isFullscreen) {
            storeCallback2(this, "event:exitFullscreen", exitFullscreen);
          } else {
            removeCallback2(this, "event:exitFullscreen", exitFullscreen);
          }
          this.ready().then(() => {
            postMessage2(this, "fullscreenchange", screenfull2.isFullscreen);
          });
        };
        screenfull2.on("fullscreenchange", this.fullscreenchangeHandler);
      }
      return this;
    }
    /**
    ...
    callMethod(name) {
      ...
      return new npo_src2((resolve, reject) => {
        return this.ready().then(() => {
          storeCallback2(this, name, {
            resolve,
            reject
          });
          if (args.length === 0) {
    ...
    get(name) {
      return new npo_src2((resolve, reject) => {
        name = getMethodName2(name, "get");
        return this.ready().then(() => {
          storeCallback2(this, name, {
            resolve,
            reject
          });
          postMessage2(this, name);
        }).catch(reject);
      });
    }
    /**
    ...
    on(eventName, callback) {
      ...
      const callbacks = getCallbacks2(this, `event:${eventName}`);
      if (callbacks.length === 0) {
        this.callMethod("addEventListener", eventName).catch(() => {
        });
      }
      storeCallback2(this, `event:${eventName}`, callback);
    }
    /**
     * Remove an event listener for the specified event. Will remove all
     * listeners for that event if a `callback` isn’t passed, or only that
    ...
    off(eventName, callback) {
      ...
      const lastCallback = removeCallback2(this, `event:${eventName}`, callback);
      if (lastCallback) {
     ...
     */
    ready() {
      const readyPromise = readyMap2.get(this) || new npo_src2((resolve, reject) => {
        reject(new Error("Unknown player. Probably unloaded."));
      });
      return npo_src2.resolve(readyPromise);
    }
    /**
     * A promise to add a cue point to the player.
     *
    ...
    requestFullscreen() {
      if (screenfull2.isEnabled) {
        return screenfull2.request(this.element);
      }
      return this.callMethod("requestFullscreen");
    }
    /**
    ...
    getFullscreen() {
      if (screenfull2.isEnabled) {
        return npo_src2.resolve(screenfull2.isFullscreen);
      }
      return this.get("fullscreen");
    }
    /**
     ...
     */
    destroy() {
      return new npo_src2((resolve) => {
        readyMap2.delete(this);
        playerMap2.delete(this.element);
        if (this._originalElement) {
          playerMap2.delete(this._originalElement);
        ...
        if (screenfull2.isEnabled) {
          screenfull2.off("fullscreenchange", this.fullscreenchangeHandler);
        }
        resolve();
      });
    }
     ...
     */
    getColors() {
      return npo_src2.all([this.get("colorOne"), this.get("colorTwo"), this.get("colorThree"), this.get("colorFour")]);
    }
    /**
    ...
    setColors(colors) {
      if (!Array.isArray(colors)) {
        return new npo_src2((resolve, reject) => reject(new TypeError("Argument must be an array.")));
      }
      const nullPromise = new npo_src2((resolve) => resolve(null));
      ...
      return npo_src2.all(colorPromises);
    }
    /**
     * A representation of a cue point.
     *
    ...
    async setTimingSrc(timingObject, options) {
      ...
      postMessage2(this, "notifyTimingObjectConnect");
      connector.addEventListener("disconnect", () => postMessage2(this, "notifyTimingObjectDisconnect"));
      return connector;
    }
  };
  if (!isServerRuntime) {
    screenfull2 = initializeScreenfull2();
    initializeEmbeds2();
    resizeEmbeds2();
    initAppendVideoMetadata2();
    checkUrlTimeParam();
    updateDRMEmbeds();
  }
  var player_es_default = Player2;
  ...
  function initHeroSlider() {
    ...
    splide.on("video:play", () => {
      ...
      const iframe = getSlide(splide.index)?.querySelector('iframe[src*="vimeo"]');
      if (iframe) {
        activePlayer = new player_es_default(iframe);
...
@vimeo/player/dist/player.es.js:
  (*! @vimeo/player v2.30.4 | (c) 2026 Vimeo | MIT License | https://github.com/vimeo/player.js *)
  (*!
   * weakmap-polyfill v2.0.4 - ECMAScript6 WeakMap polyfill
   * https://github.com/polygonplanet/weakmap-polyfill
   * Copyright (c) 2015-2021 polygonplanet <polygon.planet.aqua@gmail.com>
